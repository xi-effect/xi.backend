...
class ChatManagement(EventGroup):  # self.host!!!
...
class ChatUserManagement(EventGroup):
    ...
    def on_invite_user(self, session: Session, chat_id: int, target_id: int, role: str):
        ...
        session.post(f"{self.host}/chat-temp/{chat_id}/users/{target_id}/", json={"role": role})
    ...
    def on_assign_user(self, session: Session, chat_id: int, target_id: int, role: str):
        session.put(f"{self.host}/chat-temp/{chat_id}/users/{target_id}/", json={"role": role})
