...
class Post(Base, Identifiable):
    ...
    changed = Column(DateTime, server_default=sql.func.now(), server_onupdate=sql.func.now())
    ...
    MainData = PydanticModel.column_model(id, title, description, created, changed, deleted,
                                          user_id, community_id)

    # Find a paginated list of community news
    @classmethod
    def find_by_community(cls, session: sessionmaker, community_id: int, offset: int, limit: int) -> list[Post]:
    ...
    def create(cls, session: sessionmaker, title: str, description: str | None, user_id: int,
        ...
        return super().create(session, title=title, description=description, user_id=user_id,
