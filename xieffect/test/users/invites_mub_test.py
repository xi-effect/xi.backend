...
def test_mub_invites(client: FlaskClient, mod_client: FlaskClient, list_tester):
    ...
    invite_list = list(list_tester(url, {}, 50, use_post=False))
    assert (counter := len(invite_list)) != 0
    assert_error(client, url, base_status, base_message, method="GET", **lister_data)

    # Check creating
    create_url = f"{base_url}/"
    new_invite = check_code(mod_client.post(create_url, json=invite_data))
    assert (invite_id := new_invite.get("id")) is not None
    counter += 1
    assert_error(client, create_url, base_status, base_message, **invite_data)
    ...
    assert dict_equal(invite, invite_data, "name", "limit", "accepted")
