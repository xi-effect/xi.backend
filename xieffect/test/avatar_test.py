...
def test_set_avatar_user(client: FlaskClient):  # deprecated
    ...
    assert check_status_code(client.get("/avatar/"), get_json=False).get_data() == image1 \
           and check_status_code(client.get("/users/1/avatar/"), get_json=False).get_data() == image1
    ...
    assert check_status_code(client.get("/avatar/"), get_json=False).get_data() == image2 \
           and check_status_code(client.get("/users/1/avatar/"), get_json=False).get_data() == image2

    check_status_code(client.delete("/avatar/"))
    check_status_code(client.get("/avatar/"), 404)
    check_status_code(client.get("/users/1/avatar/"), 404)
